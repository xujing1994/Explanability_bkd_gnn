...
class SuperPixDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name, num_val=5000):
        ...
        #self.train = self.train_ + self.test_[num_val:]
        num_partition = int(len(self.train_) / 10)
        self.partition = []
        for i in range(10):
            start = i * num_partition
            _partition_graphs, _partition_labels = self.train_[start:start+num_partition]
            self.partition.append(DGLFormDataset(_partition_graphs, _partition_labels))
